...
test('audit finds risky defaults', async () => {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ClawGuard-audit-'));
...
test('hardening writes safe defaults and rules', async () => {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ClawGuard-harden-'));
  await fs.writeFile(path.join(stateDir, 'openclaw.json'), JSON.stringify({
    gateway: { bind: '0.0.0.0' },
    sandbox: { mode: 'danger-full-access' },
    exec: { approvals: 'never' }
  ...
  assert.ok(result.backupDir.includes('.ClawGuard/backups'));
  ...
  assert.ok(soul.includes('ClawGuard:rules:start'));
});

test('rollback restores previous config and soul', async () => {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ClawGuard-rollback-'));
  await fs.writeFile(path.join(stateDir, 'openclaw.json'), JSON.stringify({
    gateway: { bind: '0.0.0.0' },
    sandbox: { mode: 'danger-full-access' },
    exec: { approvals: 'never' }

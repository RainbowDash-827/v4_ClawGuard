...
const TMP_WS = fs.mkdtempSync(path.join(os.tmpdir(), 'ClawGuard-perm-'));
...
test('forever-scope allow is observed by checkPermission', () => {
  clean();
  grantPermission({ tool: 'exec', params: { command: 'echo ClawGuard-permission-test' }, decision: 'allow', scope: 'forever' });
  const r = checkPermission('exec', { command: 'echo ClawGuard-permission-test' });

...
export function registerCliCommands({ program, config }) {
  ...
  root.command('install')
    ...
    .action(async () => {
      ...
      console.log('ClawGuard install completed.');
      console.log('Next: openclaw ClawGuard audit');
    });
  ...
  root.command('monitor')
    ...
    .action(async () => {
      ...
      console.log('ClawGuard drift monitor is running. Press Ctrl+C to stop.');
  ...
  root.command('status')
    ...
    .action(async () => {
      ...
      const context = await createAuditContext(stateDir, config);
      const report = await runAudit(context);
      console.log(`ClawGuard score: ${report.score}/100`);

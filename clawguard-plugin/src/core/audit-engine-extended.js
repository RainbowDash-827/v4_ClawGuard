  ...
  try {
    ...
    if (await fileExists(killSwitchPath)) {
      extendedFindings.push({
        ...
        remediation: 'To resume operations, remove kill switch file or run: npx openclaw ClawGuard resume',
        autoFixable: false,
        threat: 'none',
        references: [],
        owaspAsi: 'ASI10',
...
function buildNextStep({ autoFixable, severity, remediation, id }) {
  if (autoFixable) {
    return `Can be auto-fixed. First run \`npx openclaw ClawGuard harden\`, then re-run \`npx openclaw ClawGuard audit\` to verify ${id}.`;
  }

  if (severity === 'CRITICAL' || severity === 'HIGH') {
    return `Manual action required. Fix according to "${remediation}", then re-run \`npx openclaw ClawGuard audit\`.`;
  }

  return `After adjusting per "${remediation}", re-run \`npx openclaw ClawGuard audit\` to confirm the results.`;
}

function buildNextSteps(findings) {
  if (findings.length === 0) {
    return ['No issues found. Continue maintaining security posture and regularly run `npx openclaw ClawGuard audit`.'];
  }
  ...
  if (autoFixable.length > 0) {
    steps.push('Run `npx openclaw ClawGuard harden` for items that can be auto-fixed.');
  }
  ...
  steps.push('After fixes are complete, run `npx openclaw ClawGuard audit` to verify.');
  return steps;
}

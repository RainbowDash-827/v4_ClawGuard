/**
 * ClawGuard Budget Guard (v1.1 feature: Token Budget Control)
 *
 ...
 *
 * State file: $OPENCLAW_WORKSPACE/ClawGuard/budget.json
 *
 * Design notes:
...
function loadConfig(rulesPath = DEFAULT_RULES_PATH) {
  ...
  cachedConfig = {
    ...
    unlimited: cfg.unlimited === true && process.env.ClawGuard_BUDGET_FORCE !== '1',
...
/** Resolve the budget state file path; defaults to workspace/ClawGuard/budget.json. */
export function resolveBudgetFile(cfg = loadConfig()) {
  ...
  return path.join(ws, 'ClawGuard', 'budget.json');
}
...
export function saveBudget(budget, filePath) {
  ...
  try {
    ...
    console.error('[ClawGuard] budget-guard save failed:', err.message);
    return false;
  }
}

/**
 * ClawGuard Permission Store (v1.1 feature 2: persistent allow/deny)
 *
 * Plugin-only persistence layer for user authorization decisions on
 * tool calls. Two state files:
 *
 *   $WS/ClawGuard/permissions-session.json   — wiped on plugin start
 *   $WS/ClawGuard/permissions-forever.json   — survives across runs
 *
 ...
 * decisions out-of-band via `openclaw ClawGuard permission allow|deny`,
...
function resolveHmacKeyFile() {
  return path.join(workspaceDir(), 'ClawGuard', '.hmac-key');
}

/**
 * Return the HMAC key. Priority:
 *   1. ClawGuard_HMAC_KEY env var
 *   2. Auto-generated key persisted in $WS/ClawGuard/.hmac-key (mode 0600)
 */
function getHmacKey() {
  if (process.env.ClawGuard_HMAC_KEY) return process.env.ClawGuard_HMAC_KEY;
  ...
  try {
    ...
    try {
      ...
      console.error('[ClawGuard] permission-store: failed to persist HMAC key:', err.message);
    }
    return key;
  }
}
...
export function resolveSessionFile() {
  return path.join(workspaceDir(), 'ClawGuard', 'permissions-session.json');
}

export function resolveForeverFile() {
  return path.join(workspaceDir(), 'ClawGuard', 'permissions-forever.json');
}
...
function readStore(filePath) {
  try {
    ...
    if (raw.entries.length > 0 && !verifyHmac(raw)) {
      console.warn(`[ClawGuard] permission-store: HMAC verification failed for ${filePath} — treating as empty (possible tampering)`);
      return freshStore();
    }
    return raw;
  } catch {
...
function writeStore(filePath, store) {
  try {
    ...
    console.error('[ClawGuard] permission-store save failed:', err.message);
    return false;
  }
}

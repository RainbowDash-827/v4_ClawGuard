...
export function setDebugLogger(logger) {
  debugLogger = logger;
  if (debugLogger) {
    debugLogger.debug('[ClawGuard Logger] Debug logger initialized');
  }
}
...
async function resolveWorkspaceDir() {
  ...
  if (debugLogger) {
    debugLogger.debug('[ClawGuard Logger] Resolving workspace from candidates:', candidates);
  }
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (debugLogger) {
        debugLogger.debug('[ClawGuard Logger] ✓ Found workspace at:', candidate);
      }
      return candidate;
    } catch (error) {
      if (debugLogger) {
        debugLogger.debug('[ClawGuard Logger] ✗ Candidate not accessible:', candidate);
      }
    }
  }
  ...
  if (debugLogger) {
    debugLogger.debug('[ClawGuard Logger] Using fallback workspace:', fallback);
  }
  return fallback;
}
...
async function getTodayLogFile() {
  try {
    ...
    if (debugLogger) {
      debugLogger.debug('[ClawGuard Logger] Creating log directory:', logDir);
    }
    ...
    if (debugLogger) {
      debugLogger.debug('[ClawGuard Logger] Log file path:', logFile);
    }
    
    return logFile;
  } catch (error) {
    console.error('[ClawGuard] Error resolving log file:', error.message);
    if (debugLogger) {
      debugLogger.error('[ClawGuard Logger] getTodayLogFile error:', error.message, error.stack);
    }
    throw error;
  }
}
...
async function logEvent(eventType, eventData = {}) {
  try {
    ...
    if (debugLogger) {
      debugLogger.debug(`[ClawGuard Logger] ✓ Logged ${eventType} event to ${logFile}`);
    }
  } catch (error) {
    console.error(`[ClawGuard] ✗ Failed to log ${eventType} event:`, error.message);
    if (debugLogger) {
      debugLogger.error(`[ClawGuard Logger] ✗ Logging error for ${eventType}:`, error.message);
    }
  }
}
...
export function createToolLoggerHook(logger = null) {
  ...
  return async (event, ctx) => {
    ...
    try {
      ...
      console.error('[ClawGuard] permission-store check error:', error.message);
      if (debugLogger) debugLogger.error('[ClawGuard Logger] permission-store threw:', error.message);
    }

    // Persistent deny is a hard block, regardless of any rule outcome.
    if (permResult.decision === 'deny') {
      const reason = `ClawGuard blocked tool call by persistent deny (${permResult.scope}) fp=${permResult.fingerprint}`;
      try {
      ...
      console.warn(`[ClawGuard] BLOCKED ${reason}`);
    ...
    try {
      ...
      console.error('[ClawGuard] budget-guard check error:', error.message);
      if (debugLogger) debugLogger.error('[ClawGuard Logger] budget-guard threw:', error.message);
    }

    try {
      if (!budgetResult.block) inputResult = validateToolInput(toolName, params);
    } catch (error) {
      console.error('[ClawGuard] input-validator error:', error.message);
      if (debugLogger) debugLogger.error('[ClawGuard Logger] input-validator threw:', error.message);
    }
    ...
    if (permResult.decision === 'allow' && !budgetResult.block && !inputResult.block) {
      ...
      console.warn(`[ClawGuard] PERMITTED ${toolName} via persistent allow (${permResult.scope}) fp=${permResult.fingerprint}`);
      return {};
    }
    ...
    if (!inputResult.block && permResult.decision !== 'allow') {
      try {
        ...
        console.error('[ClawGuard] path-guard error:', error.message);
        if (debugLogger) debugLogger.error('[ClawGuard Logger] path-guard threw:', error.message);
      }
    }
    ...
    if (!inputResult.block && !pathResult.block && permResult.decision !== 'allow') {
      try {
        ...
        console.error('[ClawGuard] exec-gate error:', error.message);
        if (debugLogger) debugLogger.error('[ClawGuard Logger] exec-gate threw:', error.message);
      }
    }
    ...
    try {
      if (debugLogger) debugLogger.debug('[ClawGuard Logger] Hook triggered: before_tool_call', { toolName });
      ...
      console.error('[ClawGuard] before_tool_call hook error:', error.message);
      if (debugLogger) debugLogger.error('[ClawGuard Logger] before_tool_call hook failed:', error.message);
    }

    if (budgetResult.block) {
      const reason = `ClawGuard blocked tool call: token budget exhausted (${formatBudgetSummary(budgetResult)})`;
      console.warn(`[ClawGuard] BLOCKED ${reason}`);
      return { block: true, blockReason: reason };
    }
    if (inputResult.block) {
      const reason = `ClawGuard blocked malformed tool input for '${toolName}': ${inputResult.reason}`;
      console.warn(`[ClawGuard] BLOCKED ${reason}`);
      return { block: true, blockReason: reason };
    }
    if (pathResult.block) {
      const reason = `ClawGuard blocked access to protected path (${pathResult.severity || 'HIGH'}): ${pathResult.reason || pathResult.matched}. Candidate=${pathResult.candidate} Resolved=${pathResult.resolved}`;
      console.warn(`[ClawGuard] BLOCKED ${reason}`);
      return { block: true, blockReason: reason };
    }
    if (execResult.block) {
      const reason = `ClawGuard blocked dangerous command (${execResult.severity || 'HIGH'}) [${execResult.matched}]: ${execResult.reason}. Command=${execResult.command}`;
      console.warn(`[ClawGuard] BLOCKED ${reason}`);
      return { block: true, blockReason: reason };
    }
    return {};
...
export function createMessageReceivedHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      if (debugLogger) {
        debugLogger.debug('[ClawGuard Logger] Hook triggered: message_received');
      }
      ...
      console.error('[ClawGuard] ✗ message_received hook error:', error.message);
      if (debugLogger) {
        debugLogger.error('[ClawGuard Logger] ✗ message_received hook failed:', error.message);
      }
    }
...
export function createMessageSendingHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      if (debugLogger) {
        debugLogger.debug('[ClawGuard Logger] Hook triggered: message_sending');
      }
      ...
      console.error('[ClawGuard] ✗ message_sending hook error:', error.message);
      if (debugLogger) {
        debugLogger.error('[ClawGuard Logger] ✗ message_sending hook failed:', error.message);
      }
    }
...
export function createLLMInputHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      if (debugLogger) {
        debugLogger.debug('[ClawGuard Logger] Hook triggered: llm_input');
      }
      ...
      console.error('[ClawGuard] ✗ llm_input hook error:', error.message);
      if (debugLogger) {
        debugLogger.error('[ClawGuard Logger] ✗ llm_input hook failed:', error.message);
      }
    }
...
export function createLLMOutputHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      if (debugLogger) {
        debugLogger.debug('[ClawGuard Logger] Hook triggered: llm_output');
      }
      ...
      try {
        if (event.usage) {
          ...
          if (budgetState.status === 'warn') {
            console.warn(`[ClawGuard] BUDGET WARN ${formatBudgetSummary(budgetState)}`);
          } else if (budgetState.status === 'over') {
            console.warn(`[ClawGuard] BUDGET OVER ${formatBudgetSummary(budgetState)}`);
          }
        }
      } catch (err) {
        if (debugLogger) debugLogger.error('[ClawGuard Logger] budget recordUsage failed:', err.message);
      }
      ...
      console.error('[ClawGuard] ✗ llm_output hook error:', error.message);
      if (debugLogger) {
        debugLogger.error('[ClawGuard Logger] ✗ llm_output hook failed:', error.message);
      }
    }
...
export function createBeforeAgentReplyHook(logger = null) {
  ...
  return async (event, ctx) => {
    ...
    try {
      ...
      if (debugLogger) debugLogger.error('[ClawGuard Logger] before_agent_reply budget check failed:', err.message);
    }

    if (!state.block) return;

    const summary = formatBudgetSummary(state);
    const text = `⛔ ClawGuard: token budget exhausted (${summary}). Agent halted for this turn. Reset the budget file or wait for the next window to resume.`;
    console.warn(`[ClawGuard] BLOCKED LLM reply: ${summary}`);
    ...
    try {
      ...
      if (debugLogger) debugLogger.error('[ClawGuard Logger] blocked_agent_reply log failed:', err.message);
    }

    return {
      handled: true,
      reason: 'ClawGuard-budget-exhausted',
...
export async function readLogFile(filename) {
  ...
  try {
    ...
    console.error('[ClawGuard] Failed to read log file:', error.message);
    return [];
  }
}

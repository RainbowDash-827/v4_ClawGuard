# ClawGuard: Plugin-based Protection
...
ClawGuard incorporates existing security plugins and integrates relevant security functions to provide robust runtime enforcement against adversarial actions at the runtime level.
...
npx openclaw ClawGuard audit --json

# Show current security score and top threats
npx openclaw ClawGuard status
```

### Security Hardening
```bash
# Apply all available security hardening fixes
npx openclaw ClawGuard harden

# Restore configuration from latest backup
npx openclaw ClawGuard rollback

# Restore from specific backup
npx openclaw ClawGuard rollback <backup-folder-name>
```

### Monitoring
```bash
# Start real-time configuration drift monitoring
npx openclaw ClawGuard monitor
```

### Event Logs & Behavior Scanning
```bash
# View today's event logs
npx openclaw ClawGuard logs

# View logs from specific date
npx openclaw ClawGuard logs --date 2026-03-14

# Filter logs by event type (before_tool_call, message_received, message_sending, llm_input, llm_output)
npx openclaw ClawGuard logs --type before_tool_call

# Filter logs by tool name (only for before_tool_call events)
npx openclaw ClawGuard logs --tool bash

# Scan logs for security risks
npx openclaw ClawGuard logs --scan

# Scan logs and save detailed report to file
npx openclaw ClawGuard logs --scan --save-report

# List all available log files
npx openclaw ClawGuard logs --all

# Show path to today's log file
npx openclaw ClawGuard log-path

# Limit output to specific number of records (default: 20)
npx openclaw ClawGuard logs --limit 50
```

### Skill Security Scanning
```bash
# Scan a skill for security issues and risks
npx openclaw ClawGuard scan-skill ~/path/to/skill

# Output scan results as JSON
npx openclaw ClawGuard scan-skill ~/path/to/skill --json
```
...
Before (left) vs. after (right) installing ClawGuard.
OpenClaw outputs the password directly, whereas ClawGuard identifies and blocks sensitive information.
...
ClawGuard generates a comprehensive threat detection report for OpenClaw, providing a security score, categorized risks, and identifying a high-severity network misconfiguration. The report also enables automatic remediation by supplying diagnostic evidence and executable hardening commands.
...
ClawGuard performs retrospective security auditing by analyzing full-lifecycle logs and successfully detects a prompt injection attack originating from an external messaging platform.
...
Run
```bash
npx openclaw ClawGuard scan-skill ~/path/to/unsafe-skill
```
...
ClawGuard consists of multiple core layers:
